...
namespace SobekCM.METS_Editor.BatchImport
{
	...
	public class Results_Form : Form
	{
        ...
        private MenuItem detailsMenuItem_ContextMenuItem;
        private MenuItem commentsMenuItem_ContextMenuItem;

        private MenuItem actionMI_ShowDetails;        
        ...
        public Results_Form(DataTable resultSet, Importer_Type_Enum importerType, bool preview_mode )            
		{
			...
			customGrid_Panel1.Style.Default_Column_Color = Color.LightBlue;
            ...
            customGrid_Panel1.Double_Clicked += customGrid_Panel1_Double_Clicked;
            customGrid_Panel1.ContextMenu = contextMenu1;
            ...
            switch (importerType)
            {
                case Importer_Type_Enum.MARC:
                    ...
                    detailsMenuItem.Text = "Show MARC XML";
                    detailsMenuItem_ContextMenuItem.Text = "Show MARC XML";
                    Text = "MARC21 Batch Processor - Results Form";
                    break;
                ...
                case Importer_Type_Enum.Spreadsheet:
                    ...
                    // set text on menu item
                    detailsMenuItem.Text = "Show METS";
                    detailsMenuItem_ContextMenuItem.Text = "Show METS";
                    if ( importerType == Importer_Type_Enum.METS )
                        Text = "METS Importer - Results Form";
                    else
                        Text = "SpreadSheet Batch Processor - Results Form";
                    break;
            }
		...
		private void InitializeComponent()
		{
            ...
            this.customGrid_Panel1 = new DLC.Custom_Grid.CustomGrid_Panel();
            ...
            // 
            this.commentsMenuItem_ContextMenuItem.Index = 1;
            ...
            this.customGrid_Panel1.Selectable = true;
		...
		{
            if (customGrid_Panel1.Selected_Row.Length > 0)
            {
                if (customGrid_Panel1.Selected_Row[0]["Related_File"].ToString().ToUpper().IndexOf(".METS") > 0)
                {
                    // Display the METS file                   
                    Open_METS(customGrid_Panel1.Selected_Row[0]["Related_File"].ToString());
                }
                else
                {
                    Process runFile = new Process();
                    runFile.StartInfo.FileName = customGrid_Panel1.Selected_Row[0]["Related_File"].ToString();
                    runFile.Start();
                }
            }
            else
        ...
        private void commentsMenuItem_Click(object sender, EventArgs e)
        {
            if (customGrid_Panel1.Selected_Row.Length > 0)
            {
                if (customGrid_Panel1.Selected_Row[0]["Comment"].ToString().Length > 0)
                {

                    // parse the comment strings from the data row
                    string[] comments_array = customGrid_Panel1.Selected_Row[0]["Comment"].ToString().Split(new[] { ';', '.' });
        ...
        private void customGrid_Panel1_Double_Clicked(DataRow thisRow)
        {
            if (thisRow != null)
            {
                ...
                switch (importerName)
                {
                    case "MARC_IMPORTER":
                        ...
                        try
                        {
                            ...
                            row_position = Convert.ToInt32(customGrid_Panel1.Selected_Row[0]["RecordIndex"].ToString());
                        }
                        catch { }
                    ...
                    case "SPREADSHEET_IMPORTER":
                        // Display the METS file                   
                        Open_METS(customGrid_Panel1.Selected_Row[0]["METS_File_URL"].ToString());
                        break;
                }
            ...
            if (result == DialogResult.OK)
            {
                switch (saveFileDialog1.FilterIndex)
                {
                    case 1:
                        ...
                        if (customGrid_Panel1.Export_as_Text(saveFileDialog1.FileName, ','))
                        {
                    ...
                    case 3:
                        ...
                        customGrid_Panel1.Export_as_Excel(saveFileDialog1.FileName, "Batch Record Importer", "RESULTS");
                        break;
        ...
        private void Open_METS(string mets_filepath)
        {
            ...
            if (mets_filepath.Length > 0)
            {
                ...
                {                 
                    ...
                    FileInfo fileInfo = new FileInfo(mets_filepath);
                ...
                {
                    ErrorMessageBox.Show("Error encountered while opening METS file!" + ex.Message, "Unexpected Error", ex);
                }
