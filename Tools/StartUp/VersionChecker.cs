	...
	public class VersionChecker
	{
		...
		public VersionChecker()
		{
			...
			try
			{
				...
				{
					// Get this HTML page
					string html = GetHtmlPage( VersionConfigSettings.VersionCheckingXML );

					html = html.Trim();

					XmlDataDocument xmlWebDoc = new XmlDataDocument();
					xmlWebDoc.DataSet.ReadXml( new StringReader( html ));
					versioning = xmlWebDoc.DataSet.Tables[0];
				}
				else
				{
					...
					xmlDataDoc.DataSet.ReadXml( VersionConfigSettings.VersionCheckingXML + "\\VersioningDB.xml" );

					// Pull out the Data Table and assign it to the local variable
					versioning = xmlDataDoc.DataSet.Tables[0];
				}
		...
		public bool UpdateExists( )
		{
			...
			try
			{
				...
				if ( !VersionConfigSettings.AppVersion.Equals( selected[0]["NewVersion"].ToString() ) )
					return true;
				else
					return false;
			}
		...
		private void CreateXML( )
		{
			...
			DataSet ds = new DataSet("VersionChecker");
			ds.Namespace = "DLC";
